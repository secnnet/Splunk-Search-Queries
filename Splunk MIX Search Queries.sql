...
index=security sourcetype=WinEventLog:Security EventCode=4625 | stats count by host

Search for suspicious network activity:
index=network src_port=80 OR src_port=443 | stats count by src_ip, dest_ip

Search for malware activity:
index=security sourcetype="wineventlog:security" EventCode=4688 Image_Path=\AppData\Roaming\.exe | stats count by host

Search for failed VPN logins:
index=vpn_log VPN_Status=failure | stats count by VPN_Username

Search for suspicious DNS activity:
index=dns query_type=A NOT query_name=*.local | stats count by query
...
index=* sourcetype="mail" (subject="password" OR subject="account")
...
index=* sourcetype="WinEventLog:Security" (EventCode=4663 OR EventCode=4656)
| stats count by object_path, dest_user
| where count > 1000
...
index=* sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventCode=1 Image="\AppData\Roaming\.exe"
...
index=* sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventCode=10 TargetImage="*\lsass.exe"
...
index=* sourcetype="linux_secure" (message="Failed password" OR message="Invalid user")
| stats count by src_ip, user
| where count > 100

Detecting Malicious JavaScript:
index=* sourcetype="bro_http" method="GET"
| rex field=uri "..js"
| search uri="http"
...
index=* sourcetype="WinEventLog:Security" (EventCode=4688 OR EventCode=5140)
...
index=* sourcetype="msx" OR sourcetype="maillog"
| search messageid=""
...
index=* sourcetype="WinEventLog:Security" (EventCode=4656 OR EventCode=4663)
...
index=* sourcetype="bro_conn" service=""
...
index=* sourcetype="linux_secure" (message="Failed password" OR message="Accepted publickey")
| stats count by src_ip, user
| where count > 100 AND user!="root"

Detecting Malware Infection via Registry Changes:
index=* sourcetype="WinEventLog:Security" (EventCode=4657 OR EventCode=4663)
...
index=* sourcetype="WinEventLog:Microsoft-Windows-TaskScheduler/Operational" EventCode=107
| search Task_Name="*"
...
index=* sourcetype="bro_conn" id.resp_p=443
...
Detecting Phishing Attempts:
index=* sourcetype="mail" (subject="password" OR subject="login")
| stats count by sender, recipient
| where count > 50

Detecting Malware Infection via Process Hollowing:
index=* sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" Image="*explorer.exe" EventCode=1
...
index=* sourcetype="WinEventLog:Microsoft-Windows-TaskScheduler/Operational" EventCode=106
| search Task_Name="*"
| stats count by ComputerName, Task_Name
| where count > 10
...
index=* sourcetype="WinEventLog:Microsoft-Windows-WMI-Activity/Operational"
...
Detecting DDoS Attacks:
index=* sourcetype="bro_conn" id.resp_p=80 OR id.resp_p=443
...
index=* sourcetype="WinEventLog:Microsoft-Windows-TaskScheduler/Operational" EventCode=140
| search Task_Name="*"
| stats count by ComputerName, Task_Name
| where count > 10
...
index=* sourcetype="WinEventLog:Microsoft-Windows-TaskScheduler/Operational" EventCode=141
| search Task_Name="*"
| stats count by ComputerName, Task_Name

Detecting Malware Infection via DNS Tunneling:
index=* sourcetype="bro_dns" query=""
| rex field=q "\S+ (\S+)$"
...
index=* sourcetype="*ms:o365:management:activity" Operation="SendMail"
| rex field=To "\w+@(\w+.\w+)"
...
index=* sourcetype="WinEventLog:System" EventCode=4688
...
index=* sourcetype="WinEventLog:Security" EventCode=4728 OR EventCode=4729 OR EventCode=4730 OR EventCode=4731 OR EventCode=4732 OR EventCode=4733
...
index=* sourcetype="secure" action=login
...
index=* sourcetype="WinEventLog:MSExchange Management" EventCode=1
...
index=* sourcetype="XmlWinEventLog:Microsoft-Windows-PowerShell/Operational" TaskCategory="ScriptBlockInvocation"
...
Detecting Malware Infection via Use of Windows Management Instrumentation (WMI):
index=* sourcetype="WinEventLog:Microsoft-Windows-WMI-Activity/Operational" EventCode=5858
| search Query!="SELECT * FROM Win32_Processor"
| stats count by ComputerName, Query
| where count > 10

Detecting Suspicious Network Traffic via RDP:
index=* sourcetype="rdp"
