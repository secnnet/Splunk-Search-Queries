...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4660 | regex Object_Name=".*\\.(doc|xls|pdf)$"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4624 Logon_Type=10 | regex Workstation_Name="^-\[0-9\]$"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=5145 | regex Share_Name="^\\\\[A-Za-z0-9_-]+(\\\\[A-Za-z0-9_-]+)+$"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4688 | regex New_Process_Name="cmd.exe" CommandLine=".*"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4697 | regex Service_Name=".*"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4624 Logon_Type=3 | regex Logon_Process=".*\\sscm$"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4672 | regex Privilege_List=".*SeDebugPrivilege.*"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4663 Accesses=".*ReadData.*" Object_Name="*\\lsass.exe" | regex Process_Name=".*"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4688 Parent_Process_Name!="svchost.exe" | regex Process_Name=".*CreateProcess.*"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4688 | regex Process_Command_Line=".*[A-Z]:\\\\(Windows|Program Files)\\\\.*"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4656 Accesses=".*ReadData.*" Object_Name="*\\*.encrypted" | regex Process_Name=".*"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4689 | regex Process_Name=".*socket.*"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4658 Access_Mask=".*0x100.*" Object_Type="Process" | regex Subject_Logon_Id=".*" Target_User_Name=".*"
...
index=SecnNet sourcetype="stream:DNS" | regex query=".*[a-zA-Z0-9+/]{4,}.*"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4688 | regex Image=".*\\\\(Windows|Program Files)\\\\.*" Target_Image=".*\\\\(Windows|Program Files)\\\\.*"
...
index=SecnNet sourcetype="stream:DNS" | regex answer="^.*$|^(?:(?!<your_ip_address>).)*$"
...
index=SecnNet sourcetype="stream:tcp" | regex dest_ip="^((?:(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?))|(?:(?:[a-fA-F0-9]{1,4}:){7}[a-fA-F0-9]{1,4}))$"
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-PowerShell/Operational" | regex Message=".*CommandLine:.*"
...
index=SecnNet sourcetype="stream:DNS" | regex query=".*[^a-zA-Z0-9.]+.*"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4728 | regex Target_User_Name=".*"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4688 | regex New_Process_Name=".*C:\\Users\\Public\\malware.exe.*"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4625 Logon_Type=10 | stats count by src_ip, user | where count > 10
...
index=SecnNet sourcetype="access_combined_wcookie" | regex uri=".*(\\%27)|(\\')|(\\%3B)|(;).*(\\%3D)|[=].*"
...
index=SecnNet sourcetype="stream:tcp" dest_ip="*:*" dest_port="80|443|8080" | regex dest_ip="^((?:(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?))|(?:(?:[a-fA-F0-9]{1,4}:){7}[a-fA-F0-9]{1,4}))$"
...
index=SecnNet sourcetype="access_combined_wcookie" | regex uri=".*(\\%3C)|(\\<)|(<).*(\\%3E)|(\\>)|(>).*(\\%3C)|(<).*(\\%3E)|(\\>)|(>).*"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4625 | stats count by src_ip, user | where count > 10 | lookup common_passwords user | where common_passwords != ""
...
index=SecnNet sourcetype="stream:tcp" dest_port="6667|6697" | regex dest_ip="^((?:(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?))|(?:(?:[a-fA-F0-9]{1,4}:){7}[a-fA-F0-9]{1,4}))$"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4663 | regex Object_Name=".*" Accesses=".*WRITE_DAC.*" | stats count by Subject_User_Name, Object_Name | where count > 10
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4663 | regex Object_Name=".*\\\\.*\\..{3,4}" Accesses=".*WriteData.*" | stats count by Process_Name, Object_Name | where count > 10
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4688 | regex Image=".*\\\\(Windows|Program Files)\\\\.*" CommandLine=".*powershell.exe.*-EncodedCommand.*"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=10 | regex EventData=".*\\b(?:lsass.exe|lsass)\b.*"
...
index=SecnNet sourcetype="WinEventLog:Windows PowerShell" SourceName="PowerShell" | regex Message=".*[A-Za-z0-9+/]{100,}.*"
...
index=SecnNet sourcetype="stream:tcp" dest_port="80|443|8080" | regex dest_ip="^[A-Za-z0-9]{32}\\.(com|net|org)$"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4624 | stats count by Subject_User_Name, Logon_Type, src_ip | where count > 10
...
index=SecnNet sourcetype="WinEventLog:Windows PowerShell" SourceName="PowerShell" | regex Message=".*(Invoke-Expression|DownloadString|IEX|Invoke-WebRequest|DownloadFile|Out-File|Set-Content|Invoke-Shellcode|Add-Type|New-Object|Net.WebClient|Get-Content|Get-ChildItem|Get-Process|Get-Item|Remove-Item|Copy-Item|Move-Item|Start-Process|Stop-Process).*"
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-TaskScheduler/Operational" TaskCategory="Task Creation" | regex Message=".*Task Scheduler successfully registered task.*"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4688 | regex Image=".*\\\\system32\\\\systeminfo.exe.*"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4625 | stats count by Subject_User_Name, src_ip | where count > 10
...
index=SecnNet sourcetype="stream:tcp" dest_port="53" | regex dest_ip="10.0.0.1|11.0.0.1|12.0.0.1"
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" ImageLoaded="*\\*\\*.dll" | regex TargetFilename=".*\\\\(temp|downloads)\\\\.*"
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventID=11 | regex TargetFilename=".*\\\\(doc|xls|ppt|pdf|jar)\\b" CommandLine=".*-noLogo.*"
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventID=8 | regex Image=".*\\\\.*" TargetImage=".*\\\\.*"
...
index=SecnNet sourcetype="stream:tcp" dest_port="80|443|8080" | regex dest_ip="^[A-Za-z0-9]{32}\\.(com|net|org)$" | regex payload=".*[A-Za-z0-9+/]{100,}.*"
...
index=SecnNet sourcetype="WinEventLog:Windows PowerShell" SourceName="PowerShell" | regex Message=".*(Invoke-Empire|Get-Empire|Import-Empire|Start-Empire).*"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=5145 | regex Message=".*Object Type:\s+File\s+New Handle ID:.*\s+Accesses:\s+0x100000.*Share Name:.*"
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventID=11 | regex TargetFilename=".*\\\\(doc|xls|ppt|pdf|txt)\\b" CommandLine=".*-noLogo.*" | regex TargetFilename=".*\\\\(docx|xlsx|pptx|pdf|txt)\\b" CommandLine=".*-Encrypt.*"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4769 | regex TargetUserName=".*" ServiceName="(?!HOST|.*\/).*" | stats count by TargetUserName, ServiceName
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventID=1 | regex Image=".*\\\\(cmd|powershell)\\.exe" CommandLine=".*-EncodedCommand|.*DownloadString|.*Invoke-Expression|.*iex|.*-Command.*"
...
index=SecnNet sourcetype="stream:tcp" dest_port="80|443" | regex payload=".*[A-Za-z0-9+/]{100,}.*"
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventID=22 | regex Image=".*\\\\system32\\\\netsh.exe" CommandLine=".*interface.*promiscuous.*"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4624 | regex Logon_Process=".*Advapi.*" Logon_Type=".*2.*" Authentication_Package=".*NTLM.*" | stats count by Logon_Account, Source_Network_Address
...
index=SecnNet sourcetype="stream:tcp" dest_port="443" | regex dest_ip="^([A-Za-z0-9]{8}\\-){4}[A-Za-z0-9]{12}\\.(com|net|org)$"
...
index=SecnNet sourcetype="stream:dns" | regex qname=".*bankofamerica|.*paypal|.*wellsfargo|.*google|.*facebook|.*apple"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4624 | stats count by Logon_Account, Source_Network_Address | where count > 1
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventID=13 | regex Image=".*\\\\.*" CommandLine=".*SeSecurityPrivilege.*"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4663 | regex Object_Name=".*\\\\(doc|xls|ppt|pdf|txt)\\b" Access_Mask=".*DELETE.*"
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventID=10 | regex Image=".*\\\\.*" SourceImage=".*\\\\.*" | stats count by Image, SourceImage
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventID=1 | regex Image=".*\\\\powershell.exe" CommandLine=".*-EncodedCommand.*"
...
index=SecnNet sourcetype="stream:dns" | regex qname=".*[A-Za-z0-9+/]{50,}.*" | regex qname=".*\\.dns\\.tunneling\\..*" | regex qname=".*\\.dns\\.data\\.exfiltration\\..*"
...
index=SecnNet sourcetype="stream:tcp" dest_port="80|443" | regex dest_ip="^([A-Za-z0-9]{8}\\-){4}[A-Za-z0-9]{12}\\.(com|net|org)$"
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventID=1 | regex Image=".*\\\\.*" CommandLine=".*-noP.*|.*-w.*"
...
index=SecnNet sourcetype="stream:http" | regex host!="^.*\\.(google\\.com|appspot\\.com)$" | stats count by clientip, host
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventID=13 | regex Image=".*\\\\.*" TargetObject=".*\\\\.*\\b" | where TargetObject like "*\\\\Microsoft\\\\Windows\\\\CurrentVersion\\\\Run\\\\*"
...
index=SecnNet sourcetype="stream:tcp" dest_port="445" | regex payload=".*[A-Za-z]:\\\\.*"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4624 | regex Logon_Process=".*Advapi.*" Logon_Type=".*10.*" | stats count by Logon_Account, Source_Network_Address
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventID=7 | regex Image=".*\\\\.*" TargetFilename!=".*\\\\System32\\\\.*|.*\\\\Windows\\\\.*"
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventID=1 | regex Image=".*\\\\.*" CommandLine=".*-e.*"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4663 | regex Object_Name=".*\\\\(doc|xls|ppt|pdf|txt)\\b" Access_Mask=".*DELETE.*" | stats count by Object_Name

PowerShell Empire: To detect attempts to use the PowerShell Empire framework for post-exploitation activities, you can use the following RegEx to search for PowerShell commands containing Empire-specific keywords:
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventID=1 | regex Image=".*\\\\powershell.exe" CommandLine=".*empire.*" | stats count by CommandLine
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4656 | regex Object_Name=".*\\\\(doc|xls|ppt|pdf|txt)\\b" Access_Mask=".*WRITE_DAC.*|.*WRITE_OWNER.*"
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4625 | stats count by Logon_Account | where count > 5
...
index=SecnNet sourcetype="stream:tcp" dest_port="3389" | regex payload=".*\buser:.*" | regex payload=".*\bpassword:.*"
...
index=SecnNet sourcetype="mail" subject!=".*Re:.*" | stats count by from, subject
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4663 | regex Object_Name=".*\\\\(keylog|keystroke)\\b" Access_Mask=".*WRITE_DAC.*|.*WRITE_OWNER.*"
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventID=1 | regex Image=".*\\\\powershell.exe" CommandLine=".*-ep.*" | regex CommandLine=".*-enc.*" | regex CommandLine=".*empire.*" | stats count by CommandLine
...
index=SecnNet sourcetype="stream:tcp" dest_port="1433" | regex payload=".*\\bselect\\b.*\\bfrom\\b.*\\bwhere\\b.*\\b(union|or)\\b.*"
...
index=SecnNet sourcetype="stream:tcp" dest_port="135" OR dest_port="445" OR dest_port="3389" | stats count by src_ip, dest_ip, dest_port
...
index=SecnNet sourcetype="stream:http" | regex user_agent=".*(curl|wget).*" | stats count by clientip, uri
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventID=1 | regex Image=".*\\\\.*" CommandLine=".*-Namespace.*" | stats count by Image, CommandLine
...
index=SecnNet sourcetype="WinEventLog:Security" EventCode=4625 | stats count by Logon_Account, Source_Network_Address | where count > 10
...
index=SecnNet sourcetype="stream:http" status="200" | regex payload=".*<script>.*" | regex payload=".*<img.*onerror=.*" | regex payload=".*<iframe>.*"
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventID=10 | regex Image=".*\\\\.*" ServiceName!=".*\\\\system32\\\\.*|.*\\\\Windows\\\\.*" | stats count by Image, ServiceName
...
index=SecnNet sourcetype="mail" | regex attachment=".*\\\\.(doc|xls|ppt|pdf|txt)\\b"

System Information Discovery: To detect attempts to gather system information for reconnaissance purposes, you can use the following RegEx to search for events where a process has executed systeminfo.exe:
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventID=1 | regex Image=".*\\\\systeminfo.exe" | stats count by Image, CommandLine
...
index=SecnNet sourcetype="mail" | regex payload=".*(http|https)://.*\\b(bank|paypal|ebay|amazon|office365|login|security|support|signin)\\b.*" | stats count by from, subject
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventID=1 | regex Image=".*\\\\cmd.exe" CommandLine=".*net user.*" | regex CommandLine=".*wmic.*" | stats count by Image, CommandLine
...
index=SecnNet sourcetype="stream:tcp" dest_port="80" | regex payload=".*\\bX-Forwarded-For\\b.*" | regex payload=".*\\bUser-Agent:.*"
...
index=SecnNet sourcetype="stream:tcp" dest_port="445" OR dest_port="139" | regex payload=".*\b(PSExec|wmic|smb|net use|schtasks|bitsadmin)\b.*"
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-WMI-Activity/Operational" EventCode=5858 | regex User=".*\\\\SYSTEM" TargetNamespace!=".*\\\\root\\\\cimv2\\\\.*" | stats count by User, TargetNamespace
...
index=SecnNet sourcetype="stream:dns" | regex query=".*\\b(hacker|malware|botnet|backdoor|data|exfiltration|command|control)\\b.*" | stats count by query, query_type
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventID=1 | regex Image=".*\\\\wmic.exe" CommandLine=".*\\/node:.*" | stats count by Image, CommandLine
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-WMI-Activity/Operational" (EventCode=5860 OR EventCode=5861) | regex User=".*\\\\SYSTEM" TargetNamespace!=".*\\\\root\\\\cimv2\\\\.*" | stats count by User, TargetNamespace, EventCode
...
index=SecnNet sourcetype="stream:dns" | regex query=".*\\b[a-z0-9]{32}\\..*\\..*\\..*\\..*\\..*\\..*\\..*\\..*\\..*\\..*\\..*\\..*\\..*\\..*\\b.*" | stats count by query
...
index=SecnNet sourcetype="stream:tcp" | regex payload=".*base64.*" | regex payload=".*xor.*"
...
index=SecnNet sourcetype="stream:tcp" dest_port="443" | regex dest_ip=".*\\b([a-f0-9]{32}|[a-f0-9]{64})\\b.*" | stats count by dest_ip, dest_port
...
index=SecnNet sourcetype="stream:dns" | regex query=".*\\b(command|control|exfiltration|malware|botnet|hacker|backdoor)\\b.*" | stats count by query
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventID=10 | regex Image=".*\\\\lsass.exe" | stats count by Image, User
...
index=SecnNet sourcetype="stream:dns" | regex query=".*\\b(base64|encryption|file|data|transfer|exfiltration|malware|botnet|hacker)\\b.*" | stats count by query
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventID=11 | regex Image=".*\\\\(taskdl.exe|taskse.exe|dfsvc.exe)" TargetFilename=".*\\.(doc|docx|xlsx|xls|ppt|pptx|pdf)" | stats count by Image, User
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventID=3 | regex Image=".*\\\\(net.exe|smb.exe)" TargetFilename=".*" | stats count by Image, User
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventID=8 | regex Image=".*\\\\(explorer.exe|svchost.exe)" TargetImage=".*" | stats count by Image, TargetImage
...
index=SecnNet sourcetype="WinEventLog:Microsoft-Windows-Sysmon/Operational" EventID=1 | regex Image=".*\\\\(powershell.exe|cmd.exe|cscript.exe|wscript.exe)" | stats count by Image, User
